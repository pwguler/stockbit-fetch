...
python3 fetch-idx-announcement.py --start-date 2023-05-16 --end-date 2024-12-31 2>&1 | tee -a "$LOG"
echo "--- news announcements 2023-05 -> 2024 ---" | tee -a "$LOG"
python3 fetch-idx-news-announcement.py --start-date 2023-05-16 --end-date 2024-12-31 2>&1 | tee -a "$LOG"

# 2) STOCK SUMMARY 2020 -> 2024
for Y in 2020 2021 2022 2023 2024; do
  echo "=== stock-summary $Y $(date -u +%T) ===" | tee -a "$LOG"
  python3 fetch-idx-stock-summary.py --start-date "${Y}-01-01" --end-date "${Y}-12-31" 2>&1 | tee -a "$LOG"
done

# 3) BROKER SUMMARY 2020 -> 2024 (collection idxbrokersummary is currently empty)
for Y in 2020 2021 2022 2023 2024; do
  echo "=== broker-summary $Y $(date -u +%T) ===" | tee -a "$LOG"
  python3 fetch-idx-broker-summary.py --start-date "${Y}-01-01" --end-date "${Y}-12-31" 2>&1 | tee -a "$LOG"
done
