...
Source: idx.co.id trading-summary/stock-summary, fetched via curl_cffi Chrome
TLS impersonation (no browser needed). No Stockbit token needed. Collection:
`idxstocksummary` (1 doc per stock per day). Supports backfill via
...
def fetch_idx_stock_summary_for_date(date):
...
def main():
    ...
    try:
        ...
        for i, date in enumerate(trading_dates, 1):
            ...
            rows = fetch_idx_stock_summary_for_date(date)
